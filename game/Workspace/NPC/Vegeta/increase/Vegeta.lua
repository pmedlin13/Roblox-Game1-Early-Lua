script.Parent.Touched:connect(function(hit)
    if hit.Parent:FindFirstChild("Humanoid") then
		...
		local incrStrenth = strengthR * 100
		
      	if strength.Value > 150000 then
			strength.Value = strength.Value + incrStrenth
			wait(2)
		elseif strength.Value < 150000 and strength.Value >= 100000 then
			strength.Value = strength.Value + incrStrenth 
			player.Character.Humanoid.Health = player.Character.Humanoid.Health - (player.Character.Humanoid.MaxHealth * 0.20)
			strength.Value = strength.Value + incrStrenth
			wait(2)
		elseif strength.Value < 100000 then
